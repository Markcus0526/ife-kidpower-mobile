...
public class WVersionManager implements IWVersionManager {
	...
	public void checkVersion() {
		...
		if (shouldCheck && currentTimeStamp > reminderTimeStamp) {
			Log.v(TAG, "checking the latest version again ...");
	...
	private void triggerChecking() {
		synchronized (this) {
			...
			if (runnableChecking == null) {
				runnableChecking = new Runnable() {
					@Override
					public void run() {
				...
				_handler.removeCallbacks(runnableChecking);
			}
			_handler.postDelayed(runnableChecking, getReminderTimer() * 60 * 1000);
		}
	}
	...
	public void checkVersionNow() {
		...
		Log.v(TAG, "the first checking the latest version ...");
	...
	private void showDialog() {
		if (activity != null && !activity.isFinishing()) {
			...
			updateReminder(getReminderTimer());
			shouldCheck = false;
		} else {
			shouldCheck = true;
		}
	...
	public String getUpdateNowLabel() {
		return updateNowLabel != null ? updateNowLabel : activity.getString(R.string.update_now);
	}
	...
	public String getRemindMeLaterLabel() {
		return remindMeLaterLabel != null ? remindMeLaterLabel : activity.getString(R.string.remind_me_later);
	}
	...
	public String getIgnoreThisVersionLabel() {
		return ignoreThisVersionLabel != null ? ignoreThisVersionLabel : activity.getString(R.string.ignore_this_version);
	}
	...
	public String getMessage() {
		...
		switch (mMode) {
			case MODE_CHECK_VERSION:
				defaultMessage = activity.getString(R.string.whats_new);
				break;
			case MODE_ASK_FOR_RATE:
				defaultMessage = activity.getString(R.string.rate_us);
				break;
		}
	...
	public String getTitle() {
		...
		switch (mMode) {
			case MODE_CHECK_VERSION:
				defaultTitle = activity.getString(R.string.new_update_available);
				break;
			case MODE_ASK_FOR_RATE:
				defaultTitle = activity.getString(R.string.rate_this_app);
				break;
		}
	...
	private void updateReminder(int reminderTimer) {
		Calendar c = Calendar.getInstance();
		long currentTimeStamp = c.getTimeInMillis();
		...
		Log.v(TAG, "updateReminder : currentTimeStamp=" + currentTimeStamp
	...
	private void setReminderTime(long reminderTimeStamp) {
		PreferenceManager.getDefaultSharedPreferences(activity)
				.edit().putLong(PREF_REMINDER_TIME, reminderTimeStamp).apply();
	}

	private void ignoreThisVersion() {
		PreferenceManager.getDefaultSharedPreferences(activity)
				.edit().putInt(PREF_IGNORE_VERSION_CODE, versionCode).apply();
	}

	private void removeIgnoredVersion() {
		PreferenceManager.getDefaultSharedPreferences(activity)
				.edit().remove(PREF_IGNORE_VERSION_CODE).apply();
	}
	...
	public String getAskForRateNegativeLabel() {
		return mAskForRateNegativeLabel == null ? activity.getString(R.string.not_now) : mAskForRateNegativeLabel;
	}
			...
			try {
				...
				if (statusCode == HttpStatus.SC_OK) {
					...
					if (responseBody != null) {
						Log.v(TAG, String.format("responseBody = %s", responseBody));
					} else {
			...
			if (statusCode != HttpStatus.SC_OK) {
				...
				try {
					...
					if (currentVersionCode < versionCode) {
						...
						if (versionCode != getIgnoreVersionCode()) {
							// check is new versionCode is ignore version
							setMessage(content);

							// show update dialog
							showDialog();

							removeIgnoredVersion();
						} else {
							Log.i(TAG, "onPostExecute: Ignored this version" + getIgnoreVersionCode());
						}
					}
