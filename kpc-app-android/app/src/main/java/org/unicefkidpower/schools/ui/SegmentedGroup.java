...
import static org.unicefkidpower.schools.R.color.kidpower_background;
...
public class SegmentedGroup extends RadioGroup {
	...
	private int mUncheckedTextColor = -1;
	...
	public SegmentedGroup(Context context) {
		...
		mTintColor = CommonUtils.getColorFromRes(resources, R.color.radio_button_selected_color);
		mStrokeColor = CommonUtils.getColorFromRes(resources, R.color.radio_button_stroke_color);
		mMarginDp = (int) getResources().getDimension(R.dimen.dimen_radio_button_stroke_border);
		mCornerRadius = getResources().getDimension(R.dimen.dimen_radio_button_conner_radius);
	...
	private void initAttrs(AttributeSet attrs) {
		...
		try {
			...
			mTintColor = typedArray.getColor(
					...
					CommonUtils.getColorFromRes(resources, R.color.radio_button_selected_color));

			mStrokeColor = typedArray.getColor(
					R.styleable.SegmentedGroup_sc_stroke_color,
					CommonUtils.getColorFromRes(resources, R.color.radio_button_stroke_color));

			mCheckedTextColor = typedArray.getColor(
					R.styleable.SegmentedGroup_sc_checked_text_color,
					CommonUtils.getColorFromRes(resources, android.R.color.white));

			mUncheckedTextColor = typedArray.getColor(
					R.styleable.SegmentedGroup_sc_unchecked_text_color,
					CommonUtils.getColorFromRes(resources, R.color.kidpower_warm_grey));
	...
	private void updateBackground(View view) {
		...
		Drawable checkedDrawable = CommonUtils.getDrawableFromRes(resources, checked).mutate();
		Drawable uncheckedDrawable = CommonUtils.getDrawableFromRes(resources, unchecked).mutate();
		...
		((GradientDrawable) uncheckedDrawable).setColor(CommonUtils.getColorFromRes(getResources(), R.color.kidpower_background));
