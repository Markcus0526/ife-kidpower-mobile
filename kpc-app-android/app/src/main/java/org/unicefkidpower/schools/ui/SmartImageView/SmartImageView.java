package org.unicefkidpower.schools.ui.SmartImageView;
	...
	public void setImage(final SmartImage image, final Integer fallbackResource, final Integer loadingResource, final SmartImageTask.OnCompleteListener completeListener) {
		...
		currentTask.setOnCompleteHandler(new SmartImageTask.OnCompleteHandler() {
			...
			public void onComplete(Bitmap bitmap) {
				if (bitmap != null) {
					if (isCircular) {
						setImageBitmap(utils.getCroppedRoundBitmap(bitmap));
					} else {
						if (isRoundRect)
							setImageBitmap(utils.getRoundedCornerBitmap(bitmap, roundSize));
						else
